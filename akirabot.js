...
const { color, bgcolor } = require('./XanarchyBot/color')
const { animesaran } = require('./XanarchyBot/animesaran')
const { animesaran2 } = require('./XanarchyBot/animesaran2')
const { help } = require('./XanarchyBot/help')
const { rules } = require('./XanarchyBot/rules')
const { sewabot } = require('./XanarchyBot/sewabot')
const { listsurah } = require('./XanarchyBot/listsurah')
const { donasi } = require('./XanarchyBot/donasi')
const { fetchJson } = require('./XanarchyBot/fetcher')
const { recognize } = require('./XanarchyBot/ocr')
...
const { ind } = require('./Xanarchy')

//apikey
const apivhtear = 'XanarchyGithub' // Vhtear Beli Sendiri Yoo
const ZeksApi = 'apivinz'
const apileys = 'demo'
const TobzKey = 'BotWeX'
const lolhuman = 'KytziaYT' //Free Trial 3 day
const onlydev = 'YTkytzia' //Free Apikey
const xteam = 'XbotBotWa' //Free Trial 3 day
const namabot = 'Xanarchy bot'
...
const xanarchy = new WAConnection()
   xanarchy.on('qr', qr => {
...
xanarchy.on('credentials-updated', () => {
	const authInfo = xanarchy.base64EncodedAuthInfo()
   console.log(`credentials updated!`)
   fs.writeFileSync('./xanarchybot.json', JSON.stringify(authInfo, null, '\t'))
})
fs.existsSync('./xanarchybot.json') && xanarchy.loadAuthInfo('./xanarchybot.json')
xanarchy.connect();


xanarchy.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await xanarchy.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
...
Pásatela bien aquí`
				...
				xanarchy.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
...
@${num.split('@')[0]}`
	...
	xanarchy.on('message-new', async (mek) => {
		try {
			...
			const q = args.join(' ')
			const botNumber = xanarchy.user.jid
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			pushname = xanarchy.contacts[sender] != undefined ? xanarchy.contacts[sender].vname || xanarchy.contacts[sender].notify : undefined
			const groupMetadata = isGroup ? await xanarchy.groupMetadata(from) : ''
			...
			const reply = (teks) => {
				xanarchy.sendMessage(from, teks, text, {quoted:mek})
			}
			const sendMess = (hehe, teks) => {
				xanarchy.sendMessage(hehe, teks, text)
			}
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? xanarchy.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : xanarchy.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
			const sendImage = (teks) => {
		    xanarchy.sendMessage(from, teks, image, {quoted:mek})
		    }
		    const costum = (pesan, tipe, target, target2) => {
			xanarchy.sendMessage(from, pesan, tipe, {quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` }}})
			}
		    const sendPtt = (teks) => {
		    xanarchy.sendMessage(from, audio, mp3, {quoted:mek})
		    }
	        /*****************END SCURITY FEATURE ********/
          ...
          const checkLimit = (sender) => {
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return xanarchy.sendMessage(from,`Su límite de solicitudes se ha agotado\n\n_Nota : El límite se restablecerá cada 21:00 horas!_`, text,{ quoted: mek})
                            xanarchy.sendMessage(from, ind.limitcount(limitCounts), text, { quoted : mek})
                            found = true
                        }
                    }
                    if (found === false) {
           ...
           const isLimit = (sender) =>{ 
              ...
              if (limits >= limitawal ) {
              	  position = true
                    xanarchy.sendMessage(from, ind.limitend(pushname), text, {quoted: mek})
                    return true
              } else {
              	_limit
                  position = true
...
// ANTI LINK GRUP
		        ...
		        xanarchy.updatePresence(from, Presence.composing)
		        ...
		        setTimeout( () => {
			        xanarchy.groupRemove(from, [kic]).catch((e)=>{reply(`*${namabot} DEBE SER ADMIN—*`)})
		        }, 3000)
		        setTimeout( () => {
			        xanarchy.updatePresence(from, Presence.composing)
...
//ANTI VIRTEX 
		...
		setTimeout( () => {
			xanarchy.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
		}, 5000)
		setTimeout( () => {
			xanarchy.updatePresence(from, Presence.composing)
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("CUIDADO DEL DISCURSO DE DONG!")
                        .then(() => xanarchy.groupRemove(from, sender))
                        .then(() => {
                            xanarchy.sendMessage(from, `*「 ANTI BADWORD 」*\nTe patearon por hablar con dureza!`, text ,{quoted: mek})
                        }).catch(() => xanarchy.sendMessage(from, `Afortunadamente, no soy administrador, si el administrador ya lo pateó!`, text , {quoted : mek}))
                } else {
			...
			switch(command) {
				...
				case 'tagsticker':
                    ...
                    if ((isMedia && !lol.message.videoMessage || isQuotedSticker) && args.length == 0) {
                        ...
                        filePath = await xanarchy.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
                        var value = args.join(" ")
                        var group = await xanarchy.groupMetadata(from)
                        ...
                        xanarchy.sendMessage(from, ini_buffer, sticker, options)
		...
		comment = args[2]
                    ...
                    xanarchy.sendMessage(from, buffer, sticker, { quoted: mek})
                    break
                    case 'artinama':
               ...
               case 'namaninja': 
				...
				if (args.length < 1) return reply(`Ejemplo: Xanarchy`)
                   ...
                   case 'tebakbendera':
					...
					setTimeout( () => {
					xanarchy.sendMessage(from, '*➸ Jawaban :* '+anu.result.name, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					xanarchy.sendMessage(from, '_10 s lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					xanarchy.sendMessage(from, '_20 s lagi_…', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					xanarchy.sendMessage(from, '_30 s lagi_…', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
					xanarchy.sendMessage(from, tebakbender, text, {quoted: mek}) // ur cods
					}, 0) // 1000 = 1s,
					break
// 
				case 'dompet':
		...
		buffer = await getBuffer(`http://api.lolhuman.xyz/api/random2/baka?apikey=${lolhuman}`)
                    xanarchy.sendMessage(from, buffer, image, { quoted: mek })
                    break
			case 'bj':
			if (isBanned) return reply(ind.baned())
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: 'HALLO ONII CHAN!!', quoted: mek })
					break
					case 'semoji':
                    ...
                    xanarchy.sendMessage(from, ini_anu, sticker, {quoted: mek})
                    break
                case 'fakedonald':
                    if (args.length == 0) return reply(`Ejemplo: ${prefix + command} Xanarchy`)
                    ini_txt = args.join(" ")
                    ini_anu = await getBuffer(`http://api.lolhuman.xyz/api/tweettrump?apikey=${lolhuman}&text=${ini_txt}`)
                    xanarchy.sendMessage(from, ini_anu, image, {quoted: mek})
                    break
                case 'faketoko':
                ...
                case 'xnxx':
                    ...
                    xanarchy.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
                    break
                   case 'pixiv':
		...
		if (isBanned) return reply(ind.baned())
					...
					var ku =  mi[Math.floor(Math.random() * mi.length)];
					nye = await getBuffer(ku)
					xanarchy.sendMessage(from, nye, image, { caption: 'OHAYO DARLING!!', quoted: mek })
					break
					case 'nsfw_avatar':
		...
		if (isBanned) return reply(ind.baned())
					...
					var ku =  mi[Math.floor(Math.random() * mi.length)];
					nye = await getBuffer(ku)
					xanarchy.sendMessage(from, nye, image, { caption: 'OHAYO ONII CHAN!!', quoted: mek })
					break
			case 'senku':
		...
		if (isBanned) return reply(ind.baned())
					...
					nye = await getBuffer(ku)
					xanarchy.sendMessage(from, nye, image, { caption: 'senku!!', quoted: mek })
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: 'kurumi chan!!', quoted: mek })
					break
				case 'nakanomiku':
		...
		if (isBanned) return reply(ind.baned())
					...
					var ku =  mi[Math.floor(Math.random() * mi.length)];
					nye = await getBuffer(ku)
					xanarchy.sendMessage(from, nye, image, { caption: 'miku chan!!', quoted: mek })
					break
			case 'wibu':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: wibu})
					break
			case 'quotes2':
	...
	if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ...
	  reply(ind.wait())
	  owgi = await xanarchy.downloadAndSaveMediaMessage(ted)
	  tels = body.slice(14)
	  anu = await imgbb("3ea1465ef91578a90ee81f7d41c59a1f", owgi)
	  hehe = await getBuffer(`https://videfikri.com/api/textmaker/facebookprof/?urlgbr=${anu.display_url}&text=${tels}`)
	 xanarchy.sendMessage(from, hehe, image, {quoted:mek})
	} else {
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 xanarchy.sendMessage(from, hedhe, image, {quoted:mek})
} else {
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
  ...
  reply(ind.wait())
  owgi = await xanarchy.downloadAndSaveMediaMessage(ger)
  anu = await imgbb("08579d070df9a07cb1c2ee565aece767", owgi)
...
xanarchy.sendMessage(from, nobg, sticker, {
  quoted: mek
})
fs.unlinkSync(rano)
  })
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 xanarchy.sendMessage(from, hehpe, image, {quoted:mek})
} else {
  reply('Responde a la imagen!')
}
break
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 xanarchy.sendMessage(from, dhehe, image, {quoted:mek})
} else {
  reply('Responde a la imagen!')
}
break
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 xanarchy.sendMessage(from, hehet, image, {quoted:mek})
} else {
  reply('Responde a la imagen!')
}
break
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 xanarchy.sendMessage(from, hehre, image, {quoted:mek})
} else {
  reply('Responde a la imagen!')
}
break
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 xanarchy.sendMessage(from, hehae, image, {quoted:mek})
} else {
  reply('Responde a la imagen!')
}
break
	...
	break
		...
		if (isBanned) return reply(ind.baned())
				husw = body.slice(7)
					...
					xanarchy.sendMessage(from, bufferjj, document, {mimetype: 'document/pdf', quoted: mek})
					break
			case 'ramaljadian':
		...
		if (isBanned) return reply(ind.baned())
				...
				reply(ind.wait())
					...
					xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: '*MEME🗿*'})
					await limitAdd(sender)
					break 
			case 'galaxstyle':
		...
		if (isBanned) return reply(ind.baned())
				...
				xanarchy.sendMessage(from, attp2, sticker, {quoted: mek})
				break
			case 'thunder':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    xanarchy.sendMessage(from, anu, sticker, { quoted: mek })
                    break
		...
		if (isBanned) return reply(ind.baned())
			...
			xanarchy.sendMessage(from, `${anu.result}`, text, {quoted: mek})
		...
		if (isBanned) return reply(ind.baned())
                ...
                xanarchy.sendMessage(from, buf, image, {quoted: mek, caption: "DONE BOS✓"})})
                break
				case 'halloween':
		...
		if (isBanned) return reply(ind.baned())
				...
				case 'wame':
                 ...
                 if (!isRegistered) return reply( ind.noregis())
  					...
  					xanarchy.sendMessage(from, options, text, { quoted: mek } )
  					break
		    ...
		    txt = '*_[  < Xanarchy > ]_*\n\n'
                    ...
                    xanarchy.sendMessage(from, thumbnail, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "* < 𝙆𝙮𝙩𝙯𝙞𝙖 >*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('Xanarchyganz/fake.jpg')} } }, caption: txt})
                    get_audio = await getBuffer(get_result.audio[0].link)
                    xanarchy.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_info.song}.mp3`, quoted: mek })
                    break
                    case 'play':
                    ...
                    xanarchy.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_info.title}.mp3`, quoted: mek })
                    break
                case 'play2':
                    if (args.length == 0) return reply(`Ejemplo: ${prefix + command} Melukis Senja`)
                    query = args.join(" ")
                    ...
                    xanarchy.sendMessage(from, ini_buffer, image, { quoted: mek, caption: get_result.title })
                    get_audio = await getBuffer(get_result.audio)
                    xanarchy.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_result.title}.mp3`, quoted: mek })
                    get_video = await getBuffer(get_result.video)
                    xanarchy.sendMessage(from, get_video, video, { mimetype: Mimetype.mp4, filename: `${get_result.title}.mp4`, quoted: mek })
                    break
                case 'ytsearch':
                    if (args.length == 0) return reply(`Ejemplo: ${prefix + command} Melukis Senja`)
                    query = args.join(" ")
		...
		if (isBanned) return reply(ind.baned())
				...
				xanarchy.sendMessage(from, `${anu.result.hasil}`, {quoted: mek})
			await limitAdd(sender) 
			break 
			case 'qoutesnime':
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(limitend(pushname))
                   ...
                   xanarchy.sendMessage(from, pus, image, {quoted: mek})
                   break 
                  case 'playstore':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, buf, image, {quoted: mek,caption: "NIH KAK!!"})
					})})
					} catch (e) {
		...
		if (isBanned) return reply(ind.baned())
                    ...
                    xanarchy.sendMessage(from, buffer, image, { quoted: mek, caption: txt })
                    break
		...
		if (!isGroupAdmins) return reply(ind.admin())
              if (args[1]=="s") {var timer = args[0]+"000"
				...
				setTimeout( () => {
					...
					xanarchy.groupSettingChange (from, GroupSettingChange.messageSend, true);
					reply(close)
				}, timer)
				break
		case 'textdaun':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, aruga, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "* < 𝙆𝙮𝙩𝙯𝙞𝙖 >*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('Xanarchyganz/fake.jpg')} } }, caption: 'Aquí están los resultados Kak...'})
					break
                    case 'femdom':
				    try {
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					xanarchy.sendMessage(from, buf, image, {quoted: mek,caption: "KAWAII!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
						axios.get(`https://nekos.life/api/v2/img/kemonomimi`).then((res)=>{
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					xanarchy.sendMessage(from, buf, image, {quoted: mek,caption: "ONII CHAN BAKA!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					xanarchy.sendMessage(from, buf, image, {quoted: mek,caption: "*RECUERDA QUE HAY DIOS*!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					xanarchy.sendMessage(from, buf, image, {quoted: mek,caption: "NYANG!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					xanarchy.sendMessage(from, buf, image, {quoted: mek,caption: "NIH OM!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, qute, image, { quoted: mek, caption: animesaran() })
					break
			case 'listsurah':
					
		if (isBanned) return reply(ind.baned())
					xanarchy.sendMessage(from, listsurah() , text, tescuk, cr)
					break
			case 'audio':
				xanarchy.updatePresence(from, Presence.composing) 
		...
		if (isBanned) return reply(ind.baned())
                ...
                xanarchy.sendMessage(from, ayam, audio, {mimetype: 'audio/mp3', filename: `.mp3`, quoted: mek})
				break
			case 'kdu':
				xanarchy.updatePresence(from, Presence.composing) 
		...
		if (isBanned) return reply(ind.baned())
                ...
                xanarchy.sendMessage(hasil, image, mek, '\`\`\`Husbu\`\`\`')
				break
			case 'ganteng':
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					xanarchy.sendMessage(from, buf, image, {quoted: mek,caption: "..."})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, pinehg, image, { caption: '*Wallpaper*\n\n*Hasil Pencarian : '+paper+'*', quoted: mek })
					break
			case 'amv':
				xanarchy.updatePresence(from, Presence.composing) 
		...
		if (isBanned) return reply(ind.baned())
                ...
                xanarchy.sendMessage(from, buffer, video, {mimetype: 'video/mp4', quoted: mek})
				break
			case 'runtime':
					
		if (isBanned) return reply(ind.baned())
				xanarchy.updatePresence(from, Presence.composing, cr) 
		...
		if (isBanned) return reply(ind.baned())
					...
					var ku =  mi[Math.floor(Math.random() * mi.length)];
					nye = await getBuffer(ku)
					xanarchy.sendMessage(from, nye, image, { caption: 'ram chan!!', quoted: mek })
					await limitAdd(sender)
					break
				case 'batle':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(limits.limitend(pushname))
					...
					xanarchy.sendMessage(from, ssweb, image, {quoted: mek})
					await limitAdd(sender)
					break 
				case 'afk':
                                        tels = body.slice(4)
		...
		if (isBanned) return reply(ind.baned())
                                        ...
                                        xanarchy.sendMessage(from, fku, text, {quoted: mek})
				const afk = fs.readFileSync('./dapganz/afk');
                xanarchy.sendMessage(from, afk, MessageType.image, {quoted: mek, caption: '*ESTE KAKA ES AFK NO EN INTERRUPCIÓN SÍ!!*'})
                 break 
                                       break
				case 'unafk':
                                        tels = body.slice(4)
                                        ...
                                        xanarchy.sendMessage(from, kl7, text, {quoted: mek})
                                        break
				case 'buylimit':
...
//CASE BADWORD
		...
		if (isBanned) return reply(ind.baned())
				   ...
				   if (args.length < 1) return reply('Que busco?')
					...
					xanarchy.sendMessage(from, pint, image, {caption: '*Pencarian : '+goo+'*', quoted: mek })
					await limitAdd(sender)
					break
                    case 'nsfwblowjob':
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
                    ...
                    xanarchy.sendMessage(from, buffer, image, { quoted: mek, caption: 'Nih Bro'  })
                    break
		case 'quran':
		...
		if (isBanned) return reply(ind.baned())
				...
				xanarchy.sendMessage(from, quran, text, {quoted: mek})
				await limitAdd(sender)
				break
				case 'shadow':
                case 'cup':
		...
		if (isBanned) return reply(ind.baned())
				...
				xanarchy.sendMessage(from, anu, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "* < 𝙆𝙮𝙩𝙯𝙞𝙖 >*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('Xanarchyganz/fake.jpg')} } }, caption: 'Aquí están los resultados Kak...'})
				break
				case 'hekelserti': 				
				// Subscribe Yoo
		...
		case 'shortlink6':
			...
			case 'gets':
			   if (args.length < 1) return reply('Ingrese el nombre registrado en la lista sticker')
				...
				xanarchy.sendMessage(from, result, sticker)
				break
           case 'liststicker':
				teks = '*Sticker List :*\n\n'
			...
			case 'nightcore':
					...
					media = await xanarchy.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${media} -filter:a atempo=1.06,asetrate=44100*1.25 ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(media)
						if (err) return reply('Error!')
						hah = fs.readFileSync(ran)
						xanarchy.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:false, quoted: odc,duration:99999999999999999999999})
						fs.unlinkSync(ran)
					   })
				       break 
				case 'slow':
		       ...
		       case 'bass':
	             ...
	             case 'toptt':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						xanarchy.sendMessage(from, topt, audio, {mimetype: 'audio/mp4',  quoted: { mek: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_${namabot}_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')} } }, ptt:true})
						})
						await limitAdd(sender)
					    break
		case 'delete':
		...
		if (isBanned) return reply(ind.baned())
			...
			xanarchy.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
			break
		case 'dxd':
				try {
		...
		if (isBanned) return reply(ind.baned())
						...
						xanarchy.sendMessage(from, bufferqq, image, {quoted: mek, caption: 'ezzzz'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						sa = await getBuffer(`https://i.ibb.co/JcSjmNY/IMG-20210107-WA0052.jpg`)
						xanarchy.sendMessage(from, sa, image, {quoted: mek, caption: 'Error Kak!!'})
						reply('❌ *ERROR* ❌')
					}
					break
		case 'loli3':
		...
		if (isBanned) return reply(ind.baned())
           ...
           xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: '*LOLI IS LIFE*'})
		await limitAdd(sender)
           break
		case 'waifu':
		...
		if (isBanned) return reply(ind.baned())
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
                                        ...
                                        waifu = `*${anu.desc}`
                                        xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: waifu})
                                        break
		case 'dadu':
		...
		if (isBanned) return reply(ind.baned())
			...
			xanarchy.sendMessage(from, buffer, sticker)
			fs.unlinkSync(rano)
			})
			break
				case 'darkjoke': 
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				xanarchy.sendMessage(from, buffer, image, { quoted: mek, caption: 'Aquí están los resultados...' })
				break
				case 'transfer':
		...
		if (isBanned) return reply(ind.baned())
					...
					nye = await getBuffer(ku)
					xanarchy.sendMessage(from, nye, image, { caption: '*I LOVE MIKUâ™¥*', quoted: mek })
					await limitAdd(sender) 
					break 
			case 'naruto':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: '**UZUMAKI NARUTO*', quoted: mek })
					await limitAdd(sender)
					break 
				case 'minato':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: '*MINATO NAMIKAZE*', quoted: mek })
					await limitAdd(sender)
					break 
				case 'boruto':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: '*UZUMAKI BORUTO*', quoted: mek })
					await limitAdd(sender)
					break 
				case 'hinata':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: '*HINATA HYOUGA*', quoted: mek })
					await limitAdd(sender)
					break 
				case 'sasuke':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: '*SASUKE UCIHA', quoted: mek })
					await limitAdd(sender) 
					break 
					case 'bitly':
                 if (!isRegistered) return reply( ind.noregis())
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: '*SAKURA*', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'rem':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(limits.limitend(pushname))
					...
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
					xanarchy.sendMessage(from, nye, image, { caption: '*REM-CHAN*', quoted: mek })
					await limitAdd(sender) 
					break
				case 'chika':
		...
		if (isBanned) return reply(ind.baned())
					...
					ke =  sasu[Math.floor(Math.random() * sasu.length)];
					nye = await getBuffer(ke)
					xanarchy.sendMessage(from, nye, image, { caption: '*CHIKA-SAN*', quoted: mek })
					await limitAdd(sender) 
					break
				case 'kurumi2':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(limitend(pushname))
					...
					break 
					case 'stickerwm':
                    if ((isMedia && !xanarchy.message.videoMessage || isQuotedImage)) {
                        ...
                        request({
                            ...
                            xanarchy.sendMessage(from, ini_buff, sticker, { quoted: mek })
		...
		if (isBanned) return reply(ind.baned())  
				if (isLimit(sender)) return reply(limitend(pushname))
					...
					xanarchy.sendMessage(from, nye, image, { caption: '*KANEKI-KUN*', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'touka':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: '*TOUKA-CHAN*', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'rize':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: '*RIZE-CHAN*', quoted: mek })
					await limitAdd(sender) 	
					break 
				case 'Xanarchy':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: 'Xanarchy-CHAN', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'itori':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: 'ITORI-CHAN', quoted: mek })
					await limitAdd(sender) 
					break
...
xanarchy.sendMessage(from, dap, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound1':
...
xanarchy.sendMessage(from, satu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound2':
...
xanarchy.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound3':
...
xanarchy.sendMessage(from, tiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound4':
...
xanarchy.sendMessage(from, empat, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound5':
...
xanarchy.sendMessage(from, lima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound6':
...
xanarchy.sendMessage(from, enam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound7':
...
xanarchy.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound8':
...
xanarchy.sendMessage(from, lapan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound9':
...
xanarchy.sendMessage(from, bilan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound10':
...
xanarchy.sendMessage(from, puluh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break		     		
case 'sound11':
...
break		     		
		...
		if (isBanned) return reply(ind.baned())
			...
			if (args.length < 1) return reply(ind.wrongf())
					...
					xanarchy.sendMessage(from, vintage, image, {caption: 'nih anjim ${vin}', quoted: mek})
					await limitAdd(sender)
					break
					case 'summer':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, anu, image, { quoted: mek, caption: 'Aqui esta el resultado' })
					await limitAdd(sender)
					break
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, anu, image, {quoted: mek, caption: 'Aqui esta el resultadoa'})
					await limitAdd(sender)
					break
		...
		if (isBanned) return reply(ind.baned())
                ...
                var gh = body.slice(15)
					...
					xanarchy.sendMessage(from, anu, image, {caption: 'Aquí anjim', quoted: mek})
					await limitAdd(sender)
					break
                    case 'texttovn':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, anu, audio, {quoted: mek, ptt:true})
					await limitAdd(sender)
					break
                    case 'emojitopng':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, aruga, image, {quoted: mek})
					await limitAdd(sender)
					break
                    case 'emojitopng2':
		...
		case 'kisahnabi': // Like My Video
				...
				xanarchy.sendMessage(from, buffer7, image, {quoted: { mek: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_${namabot}_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')} } }, caption: teks})
				break
		...
		case 'ayatkursi': // Like My Video
				...
				xanarchy.sendMessage(from, anu, image, {quoted: mek, caption: 'Aquí está el resultado sis...'})
				await limitAdd(sender)
				break
					case 'captainamerica':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, arugazzz, image, {caption: 'Nih kak', quoted: mek})
					await limitAdd(sender)
					break
					case 'lava':
		...
		if (isBanned) return reply(ind.baned())
					...
					reply(ind.wait)
					xanarchy.sendMessage(from, atytyd, video, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "* < 𝙆𝙮𝙩𝙯𝙞𝙖 >*", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('Xanarchyganz/fake.jpg')} } }, caption: 'Aquí están los resultados Kak...'})
					await limitAdd(sender) 
					break  
					case 'pubglogo':
...
fs.unlinkSync(ranp)
nobg = fs.readFileSync(rano)
xanarchy.sendMessage(from, nobg, sticker, {quoted: mek})
fs.unlinkSync(rano)
  })
...
case 'triggered':
                    ...
                    exec(`wget "${buffer}" -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                        ...
                        xanarchy.sendMessage(from, buff, sticker, { quoted: mek })
                        fs.unlinkSync(rano)
                    })
                    break
                                             case 'drawing':
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 xanarchy.sendMessage(from, hehqe, image, {quoted:mek})
} else {
  reply('Qué imagen.')
}
break
					...
					if (Number(args[0]) === 1) {
						...
						xanarchy.sendMessage(from,`ALERTA!!! Si no es administrador, no envíe el enlace del grupo`, text)
		...
		if (isBanned) return reply(ind.baned())
                ...
                case 'slap':
					...
					xanarchy.sendMessage(from, pod, image, { quoted: mek, caption: '*Toxic*\n\n'+ ple })
					await limitAdd(sender)
					break
					case 'tampar':
		...
		if (isBanned) return reply(ind.baned())
					...
					media = await xanarchy.downloadAndSaveMediaMessage(enmedia)
					await xanarchy.updateProfilePicture(botNumber, media)
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, teks, text, {quoted: mek, detectLinks: false})
                        console.log(res)
                    })
					await limitAdd(sender)
					break 
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await xanarchy.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							xanarchy.sendMessage(_.jid, buff, image, {caption: `*「 BC GROUP 」*\n\nDel grupo : ${groupName}\nRemitente : wa.me/${(sender.split('@')[0])}\nMensaje : ${body.slice(6)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
		...
		if (isBanned) return reply(ind.baned())
					xanarchy.sendMessage(from, sewabot(prefix) , text, { quoted: mek })
					break 
				case 'pinterest': 
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					pinehg = await getBuffer(trest)
					xanarchy.sendMessage(from, pinehg, image, { caption: '*Pinterest*\n\n*Resultado de búsqueda : '+pinte+'*', quoted: mek })
					await limitAdd(sender) 
					break 
				case 'husbu':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
			   	if (!isGroup) return reply(ind.groupo())
					...
					xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: '>_<'})
					await limitAdd(sender)
					break
				case 'loli':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					xanarchy.sendMessage(from, buffer, image, { caption: 'kyaa >_< o... onii - chan >///<', quoted: mek})
					await limitAdd(sender)
					break		
				case 'nekonime':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					xanarchy.sendMessage(from, buffer, image, { caption: 'Salva a la loli, golpea al peludo', quoted: mek})
					await limitAdd(sender)
					break
				case 'neko':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					setTimeout( () => {
					xanarchy.sendMessage(from, tebak, text, {quoted: mek})
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					xanarchy.sendMessage(from, '_10 s lagi..._', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout( () => {
					xanarchy.sendMessage(from, '_20 s lagi..._', text) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
					xanarchy.sendMessage(from, '_30 s lagi..._', text) // ur cods
					}, 2500) // 1000 = 1s,
					setTimeout( () => {
					xanarchy.sendMessage(from, ngebuff, image, { caption: '_k_', quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break
				case 'ranime':
		...
		if (isBanned) return reply(ind.baned())
				...
				res = await fetchJson(`https://api.vhtear.com/randomloli&apikey=${apivhtear}`, {method: 'get'})
					...
					xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: 'No hagas ingredientes para mi tío'})
				} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
				}
		...
		if (isBanned) return reply(ind.baned())
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					xanarchy.sendMessage(from, buf, image, {quoted: mek,caption: "*RECUERDA QUE HAY UN DIOS HERMANO*"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('*ERROR*')
		...
		if (isBanned) return reply(ind.baned())
						...
						buffer = await getBuffer(res.result)
						xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: '*RECUERDA QUE HAY UN DIOS HERMANO*'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('*ERROR*')
					}
		...
		if (isBanned) return reply(ind.baned())
						...
						buffer = await getBuffer(res.result)
						xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: '*:)*'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('❌ *ERROR* ❌')
					}
		...
		if (isBanned) return reply(ind.baned())
				...
				if (!isNsfw) return reply(ind.nsfwoff())
						...
						buffer = await getBuffer(res.result)
						xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni anjim'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply(' *ERROR* ')
					}
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: '^W^'})
					await limitAdd(sender)
					break
			case 'nekopoi':
		...
		if (isBanned) return reply(ind.baned())
                    ...
                    xanarchy.sendMessage(from, ini_buffer, image, { quoted: mek })
                    break
                    case 'wikipedia':
		...
		if (isBanned) return reply(ind.baned())
			...
			if (!isNsfw) return reply(ind.nsfwoff())
					...
					reply(ind.wait())
					buffer = await getBuffer(anu.result)
					xanarchy.sendMessage(from, buffer, image, { caption: 'Comli teross', quoted: mek})
					await limitAdd(sender)
					break			
			case 'blowjob':
		...
		if (isBanned) return reply(ind.baned())
                ...
                reply(ind.wait)
                   ...
                   xanarchy.sendMessage(from, buff, image, {quoted: mek, caption: resep})
                   await limitAdd(sender) 
                   break 
                   case 'stalkig':
		...
		if (isBanned) return reply(ind.baned())
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
                    ...
                    xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: hasil})
                    await limitAdd(sender)
			        break 
			       case 'githubstalk':
		...
		case 'ytstalk':
                    ...
                    xanarchy.sendMessage(from, ini_buffer, sticker, { quoted: mek})
                    break
                    case 'kickall':
                    if (!isOwner) return reply(ind.ownerb())
			if (!isGroupAdmins) return reply(ind.admin())
					...
					xanarchy.groupRemove(from, members_id)
					break 
					case 'setreply':
					if (!isOwner) return reply(ind.ownerb())
                    			xanarchy.updatePresence(from, Presence.composing) 
					...
					xanarchy.sendMessage(from, buffer, audio, {mimetype: 'audio/mp4', filename: `masyaallah.mp3`, quoted: mek })
					break
					case 'grouplist':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, teks.trim(), extendedText, {quoted: mek})
					break
					case 'blocklist':
					...
					xanarchy.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": blocked}})
					break
		...
		case 'daftar':
                ...
                if (isGroup) {
                    ...
                    await xanarchy.sendMessage(from, pp_user, image, {quoted: mek, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					xanarchy.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					await limitAdd(sender)
					break
				case 'kapankah':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					xanarchy.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: mek })
					await limitAdd(sender)
					break
         			  case 'apakah':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					xanarchy.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: mek })
					await limitAdd(sender)
					break
				case 'rate':
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					xanarchy.sendMessage(from, 'Rate : *'+rate+'*\n\nPorcentaje : '+ te+'%', text, { quoted: mek })
					await limitAdd(sender)
					break
		case 'babi':
		...
		if (isBanned) return reply(ind.baned())
				...
				xanarchy.sendMessage(from, buffer, image, {caption: 'Aquí la hermana ya terminó..', quoted: mek})
				await limitAdd(sender)
				break
		case 'nulis3':		
	...
	xanarchy.updatePresence(from, Presence.composing)
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
			...
			reply(ind.wait)
				  ...
				  nama = tulis.split("|")[0];
					...
					xanarchy.sendMessage(from, nulis, image, {quoted: mek})
					await limitAdd(sender) 
					break  
				case 'quran':
	  ...
	  case 'seberapagay':
		...
		if (isBanned) return reply(ind.baned())
				...
				xanarchy.sendMessage(from, 'Persen Bucin Kak: *'+rate+'*\n\nJawaban : '+ pbucin +'', text, { quoted: mek })
				await limitAdd(sender) 
				break 
			case 'pantun':
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(limitend(pushname))
					...
					xanarchy.sendMessage(from, pok, image, { quoted: mek , caption: 'nihhh'})
					await limitAdd(sender) 
					break  
                case 'kucing':
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(limitend(pushname))
					...
					xanarchy.sendMessage(from, pok, image, { quoted: mek , caption: '*nyang🐈*'})
					await limitAdd(sender) 
					break 
		case 'fitnah':
		...
		if (!isGroup) return reply(ind.groupo())                 
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					var bot = gh.split("&")[2];
					xanarchy.sendMessage(from, `${bot}`, text, {quoted: { mek: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
					break
					case 'ntahlah':
		...
		if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nye, image, { caption: 'Inu!!', quoted: mek })
					break
	case 'hobby':
		...
		if (isBanned) return reply(ind.baned())
           if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					xanarchy.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
					await limitAdd(sender)
					break
				case 'meme':
		...
		if (!isGroup) return reply(ind.groupo())
					...
					setTimeout( () => {
					xanarchy.sendMessage(from, '*➸ Jawaban :* '+anu.result.jawaban+ '\n\n• Penjelasan: *'+ anu.result.desk+'*', text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					xanarchy.sendMessage(from, '_10 s lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					xanarchy.sendMessage(from, caklontong, text, {quoted: mek}) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
			case 'family100':
		...
		if (!isGroup) return reply(ind.groupo())
					...
					setTimeout( () => {
					xanarchy.sendMessage(from, '*➸ Jawaban :* '+anu.result.jawaban, text, {quoted: mek}) // ur cods
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
					xanarchy.sendMessage(from, '_10 s lagi…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					...
					setTimeout( () => {
					xanarchy.sendMessage(from, family, text, {quoted: mek }) // ur cods
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
         	case 'ping':
		...
		if (isBanned) return reply(ind.baned())
           	await xanarchy.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(time, moment())} _Second_`)
		break
               			case 'help': 
               			case 'help': 
               			case 'menu':
...
` 
		...
		case 'quotesislami':
				...
				if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, islam, image, { quoted: mek, caption: '*Quotes Islami*\n\n'+ isl })
					await limitAdd(sender)
					break	
		case 'quotesnasehat':
                 if (!isRegistered) return reply( ind.noregis())
				...
				if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, nase, image, { quoted: mek, caption: '*Quotes Nasehat*\n\n'+ nsh })
					await limitAdd(sender)
					break	
		case 'quotescinta':
                 if (!isRegistered) return reply( ind.noregis())
				...
				if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, cta, image, { quoted: mek, caption: '*Quotes Cinta*\n\n'+ cin })
					await limitAdd(sender)
					break
		// Movie & Story
                case 'lk21':
                ...
                case 'infogempa':
                    ...
                    xanarchy.sendMessage(from, get_buffer, image, { quoted: mek, caption: ini_txt })
                    break
                 case 'telesticker':
                    ...
                    for (sticker_ in ini_sticker) {
                        ...
                        xanarchy.sendMessage(from, ini_buffer, sticker)
                     }
                    break
                case 'tiktoknowm':
                    ...
                    xanarchy.sendMessage(from, ini_buffer, video, { quoted: mek })
                    break
                case 'ngif':
                case 'nsfwnekogif':
                ...
                case 'tiktokmusic':
                    ...
                    xanarchy.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, quoted: mek })
                    break
                case 'spotify':
                    ...
                    xanarchy.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.title}.mp3`, quoted: mek })
                    break
                case 'spotifysearch':
                    if (args.length == 0) return reply(`Ejemplo: ${prefix + command} Melukis Senja`)
                    query = args.join(" ")
                ...
                case 'jooxplay':
                    ...
                    xanarchy.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
                    get_audio = await getBuffer(get_result.audio[0].link)
                    xanarchy.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.info.song}.mp3`, quoted: mek })
                    break
                case 'igdl':
                    ...
                    xanarchy.sendMessage(from, ini_buffer, ini_type, { quoted: mek })
                    break
                case 'fbdl':
                ...
                case 'gimage2':
                    ...
                    for (var x = 0; x <= 5; x++) {
                        ...
                        xanarchy.sendMessage(from, ini_buffer, image)
                    }
                    break
                case 'konachan':
                    if (!isRegistered) return reply( ind.noregis())
		...
		case 'quotesmotivasi':
				...
				if (isBanned) return reply(ind.baned())
					...
					xanarchy.sendMessage(from, vass, image, { quoted: mek, caption: '*Motivasi*\n\n'+ cin })
					await limitAdd(sender)
					break
                case 'level':
		case 'lvl':
		...
		if (isBanned) return reply(ind.baned())
               ...
               xanarchy.sendMessage(from, resul, text, { quoted: mek})
                .catch(async (err) => {
                        console.error(err)
                        await reply(`Error!\n${err}`)
                    })
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					xanarchy.sendMessage(from, options, text)
					await limitAdd(sender)
					break
					case 'setmemberlimit':
					if (args.length < 1) return
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
                    ...
                    xanarchy.sendMessage(from, buffer, image, { quoted: mek, caption: 'Plastiqnya Kaka:v'  })
                    break
                    case 'exo':
		...
		if (isBanned) return reply(ind.baned())
                ...
                var gh = body.slice(12)
					...
					buffer = await getBuffer(anu.result)
					xanarchy.sendMessage(from, buffer, image, {caption: 'Nih anjim', quoted: mek})
					await limitAdd(sender)
					break
				case 'pornhub':
                case 'glitch':
		...
		xanarchy.sendMessage(from, truteh, image, { caption: '*TRUTH*\n\n'+ ttrth, quoted: mek })
		break
		case 'dare':
		...
		xanarchy.sendMessage(from, tod, image, { quoted: mek, caption: '*DARE*\n\n'+ der })
		break
		case 'ssweb':
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					xanarchy.sendMessage(from, buff, image, {quoted: mek})
					await limitAdd(sender)
					break
                case 'pokemon':
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
					...
					pok = await getBuffer(nimek)
					xanarchy.sendMessage(from, pok, image, { quoted: mek })
					await limitAdd(sender)
					break
                case 'anjing':
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
					...
					if (Number(args[0]) === 1) {
						...
						xanarchy.sendMessage(from,`ALERTA!!! ¡¡NO ENVÍE VIRUS !! PORQUE ESTE GRUPO ES ANTI VIRUS !! SI ENVÍA VIRUS SERÁ BANEADO!!`, text)
					} else if (Number(args[0]) === 0) {
					...
					xanarchy.sendMessage(from, { quoted: mek})
					break
					case 'antialakazam':
					if (!isGroup) return reply(ind.groupo())					
					if (!isBotGroupAdmins) return reply('No soy akmin 😭')					
		...
		if (isLimit(sender)) return reply(ind.limitend(pushname))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						xanarchy.sendMessage(from, bufferlkj, audio, {mimetype: 'audio/mp4', quoted: mek})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break
                ...
                xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: ingfomp3})
                xanarchy.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', quoted: mek})
                break
                case 'ytmp4':
                    ini_link = args[0]
                    ...
                    xanarchy.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: mek })
                    break
                case 'darkneon':
                case 'candlemug':
                case 'lovemsg':
		...
		if (isBanned) return reply(ind.baned())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
              	    if (args.length < 1) return reply('el texto?')
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return xanarchy.sendMessage(from, 'Teksnya kepanjangan Bambank', text, {quoted: mek})
                    buff = await getBuffer(`https://docs-jojo.herokuapp.com/api/text3d?text=${teks}`, {method: 'get'})
                    xanarchy.sendMessage(from, buff, image, {quoted: mek, caption: `${teks}`})
			     	await limitAdd(sender)
					break
			    case 'fototiktok':
		...
		if (isBanned) return reply(ind.baned())
			if (isLimit(sender)) return reply(ind.limitend(pusname))
                ...
                xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: `${body.slice(5)}`})
				await limitAdd(sender)
					break
                case 'kbbi':
		...
		if (isBanned) return reply(ind.baned())
			...
			if (!tex) return xanarchy.sendMessage(from, 'MASUKAN URL/TEKS UNTUK DI JADIKAN QR', text, {quoted: mek})
		...
		if (isBanned) return reply(ind.baned())
			...
			xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: `${teks}`})
			await limitAdd(sender)
			break
				case 'happymod':
		...
		if (isBanned) return reply(ind.baned())
				...
				if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						const media = await xanarchy.downloadAndSaveMediaMessage(encmedia)
						reply(ind.wait())
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return xanarchy.sendMessage(from, 'Código de idioma requerido!!', text, {quoted: mek})
					const gtts = require('./Xanarchy/gtts')(args[0])
					if (args.length < 2) return xanarchy.sendMessage(from, '¿Qué texto quieres que suene? es mi voz?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
					...
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							...
							buffer = fs.readFileSync(rano)
							if (err) return reply(ind.stikga())
							xanarchy.sendMessage(from, buffer, audio, {quoted: mek, ptt:true})
							fs.unlinkSync(rano)
						})
					})
					await limitAdd(sender)
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				try {
						if (args.length < 1) return xanarchy.sendMessage(from, 'Nombre del usuario?', text, {quoted: mek})
                 ...
                 case 'linkgc':
				    ...
				    if (isLimit(sender)) return reply(ind.limitend(pusname))
				    if (!isBotGroupAdmins) return reply(ind.badmin())
				    linkgc = await xanarchy.groupInviteCode (from)
				    yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
				    xanarchy.sendMessage(from, yeh, text, {quoted: mek})
			        await limitAdd(sender)
					break
				case 'tagall':
					if (!isGroup) return reply(ind.groupo())
				...
				case 'clearall':
					if (!isOwner) return reply(ind.ownerb())
					anu = await xanarchy.chats.all()
					xanarchy.setMaxListeners(25)
					for (let _ of anu) {
						xanarchy.deleteChat(_.jid)
					}
					reply(ind.clears())
					break
			       case 'block':
				 xanarchy.updatePresence(from, Presence.composing) 
				 xanarchy.chatRead (from)
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
					xanarchy.blockUser (`${body.slice(7)}@c.us`, "add")
					xanarchy.sendMessage(from, `Órdenes recibidas, bloque ${body.slice(7)}@c.us`, text)
					break
                    case 'unblock':
					if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				    xanarchy.blockUser (`${body.slice(9)}@c.us`, "remove")
					xanarchy.sendMessage(from, `Órdenes recibidas, bloque ${body.slice(9)}@c.us`, text)
					break
				case 'leave': 
				if (!isGroup) return reply(ind.groupo())
				if (!isOwner) return reply(ind.ownerb())
				await reply(from, 'bye').then(() => xanarchy.groupLeave(from))
					break 
			case 'delete':
			case 'del':
			case 'd':
			if (!isGroup) return reply(ind.groupo())
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await xanarchy.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							xanarchy.sendMessage(_.jid, buff, image, {caption: `❮ 𝙋𝙀𝙎𝘼?? 𝘽??𝙊𝘼𝘿𝘾𝘼𝙎𝙏 ❯\n\n${body.slice(4)}`})
						}
			   	...
			   	case 'setpp': 
                       ...
                       if (!isGroupAdmins) return reply(ind.admin())
                        if (!isBotGroupAdmins) return reply(ind.badmin())
                       media = await xanarchy.downloadAndSaveMediaMessage(mek)
                         await xanarchy.updateProfilePicture (from, media)
				...
				case 'add':
					...
					if (args[0] === 'buka') {
						...
						xanarchy.groupSettingChange(from, GroupSettingChange.messageSend, false)
		...
		break    
           	case 'setname':
			    ...
			    if (!isGroupAdmins) return reply(ind.admin())
				if (!isBotGroupAdmins) return reply(ind.badmin())
                xanarchy.groupUpdateSubject(from, `${body.slice(9)}`)
                xanarchy.sendMessage(from, 'Nombre del grupo cambiado con éxito', text, {quoted: mek})
					break
                case 'setdesc':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
				if (!isBotGroupAdmins) return reply(ind.badmin())
                xanarchy.groupUpdateDescription(from, `${body.slice(9)}`)
                xanarchy.sendMessage(from, 'Descripción cambiada con éxito', text, {quoted: mek})
					break
           				case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					...
					if (mentioned.length > 1) {
						...
						xanarchy.groupDemoteAdmin(from, mentioned)
					} else {
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						xanarchy.groupMakeAdmin(from, mentioned)
					} else {
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						xanarchy.groupRemove(from, mentioned)
					} else {
		...
		if (isBanned) return reply(ind.baned())
				if (!isQuotedSticker) return reply('Responda a un sticker!')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						xanarchy.sendMessage(from, buffer, image, {quoted: mek, caption: 'Así?'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
				...
				case 'clone':
					...
					try {
						pp = await xanarchy.getProfilePicture(id)
						buffer = await getBuffer(pp)
						xanarchy.updateProfilePicture(botNumber, buffer)
				...
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						reply(ind.wait())
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						media = await xanarchy.downloadMediaMessage(encmedia)
						await wait(media).then(res => {
							xanarchy.sendMessage(from, res.video, video, {quoted: mek, caption: res.teks.trim()})
						}).catch(err => {
							reply(err)
						})
					} else {
		...
		xanarchy.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
		...
		xanarchy.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			...
			if (!isGroup && !isCmd) {
                        await xanarchy.updatePresence(from, Presence.composing)
